...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Enforce VAPID keys in production unless explicitly disabled
  const pushRequired = process.env.PUSH_NOTIFICATIONS_REQUIRED !== "false";
  if (
    ...
    throw new Error(
      "🚨 CRITICAL: VAPID_PUBLIC_KEY and VAPID_PRIVATE_KEY must be set in production for push notifications.\n" +
      "Generate keys using: npx web-push generate-vapid-keys\n" +
      "Or set PUSH_NOTIFICATIONS_REQUIRED=false to run without push."
    );
  }
