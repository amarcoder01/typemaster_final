...
export function createNotificationRoutes(storage: IStorage, notificationScheduler?: NotificationScheduler) {
  ...
  router.get('/api/notifications/vapid-public-key', (req, res) => {
    ...
    res.json({ publicKey: publicKey || null, enabled: !!publicKey });
  });
  ...
  // Track click from service worker (backward-compatible endpoint)
  router.post('/api/notifications/track-click', async (req, res) => {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }

    try {
      const notificationIdRaw = req.body?.notificationId;
      const notificationId = typeof notificationIdRaw === 'number'
        ? notificationIdRaw
        : parseInt(String(notificationIdRaw), 10);
