...
export class NotificationScheduler {
  ...
  /**
   * Process due notification jobs with concurrency limiting
   * Limits concurrent DB operations to prevent overwhelming Neon serverless
   */
  private async processDueJobs() {
    ...
    try {
      ...
      // Limit concurrency to 3 to prevent overwhelming the database connection pool
      const limit = pLimit(3);
      const results = await Promise.allSettled(
        dueJobs.map(job => limit(() => this.processJobWithRetry(job)))
      );
   ...
   */
  private async processJobWithRetry(job: NotificationJob, retries = 2): Promise<void> {
    try {
      ...
      if (isTimeout && retries > 0) {
        ...
        return this.processJobWithRetry(job, retries - 1);
      }
      throw error;
    }
  }
