...
export class DatabaseStorage implements IStorage {
  ...
  async getRandomParagraphs(language: string, count: number, mode?: string, difficulty?: string): Promise<TypingParagraph[]> {
    ...
    // Final fallback to ANY language paragraphs (exclude typing-related)
    // This keeps continuous typing working even if a language has no seeded paragraphs.
    const anyConditions = [
      eq(typingParagraphs.isTypingRelated, false),
    ];
    if (mode) {
      anyConditions.push(eq(typingParagraphs.mode, mode));
    }
    if (difficulty) {
      anyConditions.push(eq(typingParagraphs.difficulty, difficulty));
    }

    const anyParagraphs = await db
      .select()
      .from(typingParagraphs)
    ...
    if (anyParagraphs.length > 0) {
      return anyParagraphs;
    }

    // Ultimate fallback to English paragraphs (exclude typing-related)
