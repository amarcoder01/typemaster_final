...
// Script detection patterns for validation
const SCRIPT_PATTERNS: Record<string, { expected: RegExp; forbidden: RegExp[] }> = {
  ...
  ru: {
    expected: /[\u0400-\u04FF]/, // Cyrillic
    forbidden: [/[\u0600-\u06FF]/, /[\u0900-\u097F]/] // Arabic, Devanagari
  },
  ja: {
    expected: /[\u3040-\u30FF\u4E00-\u9FFF]/, // Hiragana, Katakana, Kanji
    forbidden: [/[\u0600-\u06FF]/, /[\u0900-\u097F]/] // Arabic, Devanagari
  },
  ko: {
    expected: /[\uAC00-\uD7AF\u1100-\u11FF]/, // Hangul
    forbidden: [/[\u0600-\u06FF]/, /[\u0900-\u097F]/] // Arabic, Devanagari
  },
  zh: {
    expected: /[\u4E00-\u9FFF]/, // CJK
...
// Validate that content uses the correct script and doesn't mix scripts
function validateScript(content: string, language: string): { valid: boolean; error?: string } {
  const patterns = SCRIPT_PATTERNS[language];
  if (!patterns) {
    // No specific script validation for Latin-based languages
    return { valid: true };
  }

  // Check if expected script is present
  if (!patterns.expected.test(content)) {
    return { 
      valid: false, 
      error: `Content does not contain expected script for ${language}` 
    };
  }

  // Check for forbidden scripts (script mixing)
  for (const forbiddenPattern of patterns.forbidden) {
    if (forbiddenPattern.test(content)) {
      return { 
        valid: false, 
        error: `Content contains mixed scripts - found forbidden script in ${language} content` 
      };
    }
  }
...
export async function generateTypingParagraph(
  ...
  // Language-specific script instructions to prevent script mixing
  const scriptInstructions: Record<string, string> = {
    hi: "CRITICAL: Write ONLY in Devanagari script (हिंदी). Do NOT mix with any other script.",
    mr: "CRITICAL: Write ONLY in Devanagari script (मराठी). Do NOT mix with any other script.",
    ar: "CRITICAL: Write ONLY in Arabic script (العربية). Do NOT mix with any other script like Devanagari.",
    bn: "CRITICAL: Write ONLY in Bengali script (বাংলা). Do NOT mix with any other script.",
    ta: "CRITICAL: Write ONLY in Tamil script (தமிழ்). Do NOT mix with any other script.",
    te: "CRITICAL: Write ONLY in Telugu script (తెలుగు). Do NOT mix with any other script.",
    ja: "CRITICAL: Write ONLY in Japanese scripts (Hiragana/Katakana/Kanji). Do NOT mix with other scripts.",
    zh: "CRITICAL: Write ONLY in Chinese Simplified characters (简体中文). Do NOT mix with other scripts.",
    ko: "CRITICAL: Write ONLY in Korean Hangul (한글). Do NOT mix with other scripts.",
    ru: "CRITICAL: Write ONLY in Cyrillic script (Русский). Do NOT mix with other scripts.",
    th: "CRITICAL: Write ONLY in Thai script (ไทย). Do NOT mix with other scripts.",
  };
  
  const getScriptNote = (lang: string): string => {
    if (lang === 'en') return '';
    if (scriptInstructions[lang]) {
      return ` ${scriptInstructions[lang]}`;
    }
    // For Latin-based languages, no special script instruction needed
    return '';
  };
...
- Structure should be sophisticated: introduce complex ideas, analyze deeply, draw connections`
  ...
  if (customPrompt) {
    ...
    const scriptNote = getScriptNote(language);
    
    prompt = `Write a ${difficulty}-level paragraph in ${languageName} about: "${customPrompt}".
...
6. Write ENTIRELY in ${languageName} - do not mix languages or scripts

${difficulty.toUpperCase()} DIFFICULTY GUIDELINES:
${difficultyGuidelines[difficulty]}

Return ONLY the paragraph text in ${languageName}, no explanations or meta-commentary.`;
  } else {
...
7. Write ENTIRELY in ${languageName} - do not mix languages or scripts${generalModeGuidance}
...
MULTI-LANGUAGE & SCRIPT RULES (CRITICAL):
- Write ONLY in the specified target language
- NEVER mix scripts or languages - if asked for Arabic, write 100% in Arabic script
- NEVER mix Devanagari with Arabic, or any other script combinations
- Each language has its own script - use ONLY that script:
  * Arabic (ar) = Arabic script (العربية)
  * Hindi (hi) = Devanagari script (हिंदी)
  * Marathi (mr) = Devanagari script (मराठी)
  * Bengali (bn) = Bengali script (বাংলা)
  * Tamil (ta) = Tamil script (தமிழ்)
  * Telugu (te) = Telugu script (తెలుగు)
  * Japanese (ja) = Japanese scripts only
  * Chinese (zh) = Chinese characters only
  * Korean (ko) = Hangul only
  * Russian (ru) = Cyrillic only
- Latin-based languages (English, Spanish, French, etc.) use Latin alphabet

CULTURAL ADAPTATION:
- Adapt content to be culturally relevant and respectful for the target language
- Use examples and contexts that resonate with speakers of that language
- Write with cultural sensitivity and awareness for native speakers

CRITICAL RULES:
1. NEVER mention typing, keyboards, typing practice, typing speed, accuracy, or any typing-related concepts
2. Focus ONLY on the specified topic - write informative, educational content
...
6. NEVER mix different writing scripts - this is a critical error
...
Your role is to educate and engage readers with interesting, valuable content in the EXACT language and script requested.`;
  ...
  try {
    ...
    while (attempt < maxRetries) {
      ...
      // Validate script is correct (no mixing)
      const scriptValidation = validateScript(content, language);
      if (!scriptValidation.valid) {
        console.warn(`⚠️ Script validation failed: ${scriptValidation.error}`);
        console.warn(`Content sample: ${content.substring(0, 200)}...`);
        if (attempt === maxRetries) {
          throw new Error(`Script mixing detected: ${scriptValidation.error}`);
        }
        continue;
      }
      ...
      console.log(`✅ Content validated - no typing-related terms, correct script`);
      return content;
    }
